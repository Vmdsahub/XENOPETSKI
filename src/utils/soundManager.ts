...
/**
 * Advanced spaceship movement sound system - clean, futuristic, velocity-based
 */
let currentMovementSound: {
  stop: () => void;
  updateVelocity: (velocity: number, maxVelocity: number) => void;
} | null = null;

const createSpaceshipMovementSound = () => {
  try {
    const audioContext = new (window.AudioContext ||
      (window as any).webkitAudioContext)();
    ...
    // Create oscillators for clean, crystalline futuristic sound
    ...
    // Create filter for clean, crisp sound
    const filter = audioContext.createBiquadFilter();
    ...
    // Configure filter for crystalline, clean sound
    filter.type = "highpass";
    filter.frequency.setValueAtTime(150, startTime);
    filter.Q.setValueAtTime(0.7, startTime);

    // Configure oscillators for futuristic movement sound
    mainOsc.type = "sine"; // Clean sine wave for main tone
    harmonicOsc.type = "triangle"; // Soft triangle for harmonics
    subOsc.type = "sine"; // Sub-harmonic for depth

    // Base frequencies - clean and minimal
    mainOsc.frequency.setValueAtTime(200, startTime);
    harmonicOsc.frequency.setValueAtTime(400, startTime); // Perfect octave
    subOsc.frequency.setValueAtTime(100, startTime); // Sub-harmonic

    // Create subtle LFO for gentle modulation
    ...
    lfo.type = "sine";
    lfo.frequency.setValueAtTime(1.5, startTime); // Very slow modulation
    lfoGain.gain.setValueAtTime(3, startTime); // Subtle modulation depth

    // Initial volumes - very quiet for minimal aesthetic
    mainGain.gain.setValueAtTime(0, startTime);
    harmonicGain.gain.setValueAtTime(0, startTime);
    subGain.gain.setValueAtTime(0, startTime);

    // Master volume starts at 0
    masterGain.gain.setValueAtTime(0, startTime);

    // Start all oscillators
    const oscillators = [mainOsc, harmonicOsc, subOsc, lfo];
    oscillators.forEach((osc) => osc.start(startTime));

    // Return control object with velocity update function
    return {
      updateVelocity: (velocity: number, maxVelocity: number) => {
        try {
          ...
          // Smooth velocity-based volume control
          const targetVolume = normalizedVelocity * 0.08; // Very quiet for minimal aesthetic
          masterGain.gain.linearRampToValueAtTime(
            targetVolume,
            currentTime + 0.1,
          );

          // Velocity-based frequency modulation (subtle pitch changes)
          const baseFreq = 200;
          const maxFreqIncrease = 80; // Subtle frequency change
          const targetMainFreq =
            baseFreq + normalizedVelocity * maxFreqIncrease;
          ...
          // Velocity-based gain distribution for dynamic timbre
          const mainVol = 0.06 * normalizedVelocity;
          const harmonicVol = 0.03 * normalizedVelocity;
          const subVol = 0.04 * normalizedVelocity;

          mainGain.gain.linearRampToValueAtTime(mainVol, currentTime + 0.1);
          harmonicGain.gain.linearRampToValueAtTime(
            harmonicVol,
            currentTime + 0.1,
          );
          subGain.gain.linearRampToValueAtTime(subVol, currentTime + 0.1);

          // Dynamic filter cutoff based on velocity for more brightness at higher speeds
          const filterFreq = 150 + normalizedVelocity * 100;
          filter.frequency.linearRampToValueAtTime(
            filterFreq,
            currentTime + 0.15,
          );
        } catch (error) {
          console.warn("Movement sound velocity update failed:", error);
        }
      },
      ...
      stop: () => {
        try {
          ...
          console.warn("Movement sound stop failed:", error);
        }
      },
    };
  } catch (error) {
    console.warn("Movement sound creation failed:", error);
    return {
      updateVelocity: () => {},
      stop: () => {},
    };
